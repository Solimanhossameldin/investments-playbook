...
function briefLate(briefs, site) {
  const st = briefStatus(briefs);
  if (!st.behind) return "";
  const byEmail = !!(site && site.brief && site.brief.byEmail);
  const what = st.none
    ? "No issue has been archived here yet."
    : `The most recent issue on this page is ${briefLabel(st.latest)}, ${st.weekdaysBehind} weekdays ago.`;
  // Two different failures, and saying the wrong one is worse than saying
  // nothing. If the brief is going out by email, publication is fine and the
  // archive is what is behind. If it is not, publication itself has stopped.
  return byEmail
  ...
  const briefBlock = brief
    ...
    ${
      ...
      <div class="gate__box" data-gate-box>
        ...
        ${briefForm(site, "brief-form", nextLine, "", "home-gate")}
      </div>
    </div>`
        : ""
    }
...
export function briefIndex({ site, briefs }) {
  ...
  ${briefLate(briefs, site)}
  <div class="gate__box" id="get" style="max-width:560px;margin-bottom:44px"><h2>Get it in your inbox</h2><p>Free. Unsubscribe in one click.</p>${briefForm(site, "arch-form", nextLine, "", "brief-index")}</div>
...
export function briefPage({ site, brief, prev, next, briefs = [], playbooks = [] }) {
  ...
  // An archive note is not a correction. Nothing in the issue is wrong; the
  // issue simply was not written on the day it covers. It is rendered on the
  // issue so a reader can never mistake a reconstruction for a call made at
  // the time, and it is deliberately NOT read by The Record, which counts
  // corrections only.
  const archival = brief.archivalNote
    ? `<div style="max-width:var(--prose);margin:26px 0 0;padding:14px 18px;border-left:3px solid var(--hair-light);background:rgba(127,127,127,.06)"><p style="margin:0;font-size:13px;line-height:1.65"><strong>Archive note.</strong> ${esc(copy(brief.archivalNote))}</p></div>`
    : "";
...
${captureBlock(site, { source: "playbooks-index", heading: "Take the whole library with you", blurb: "All of it as one document, free. One email address, no card, unsubscribe in one click." })}
</div></section>`;
...
export function playbookPage({ site, pb, calcName, related = [], briefs = [], liveBand = "" }) {
  ...
  <div class="rail__box"><h2>The daily brief</h2><p>Three minutes on global markets and property, ${esc(cad.phrase)}.</p><a class="btn btn--ghost btn--sm" href="/brief/">Read the latest issue</a></div>
</aside>`;
  ...
  <div class="article">
    ...
    ${captureBlock(site, {
      ...
      blurb: "Every framework on this site as one document, free, plus the brief. One email address, no card, unsubscribe in one click.",
    })}
  </div>
  ${rail}
  </div>
