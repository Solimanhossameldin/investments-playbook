...
{
  "order": 44,
  "slug": "mortgage-capacity",
  "title": "Mortgage capacity, and what the bank is actually testing",
  "category": "property",
  "tier": 1,
  "calculator": "net-rental-yield",
  "reviewed": "28 August 2026",
  "summary": "Mortgage capacity is set by two tests applied at once, a loan to value cap that limits the loan against the property and a debt burden ratio that limits it against your income, and the smaller of the two answers is the only one that matters.",
  "formula": "Test one, the property\n  Maximum loan = Price x LTV cap\n  Deposit needed = Price - Maximum loan\n                 + all acquisition costs, which cannot be borrowed\n\nTest two, the borrower\n  Maximum total monthly debt = Monthly income x DBR cap\n  Room for this mortgage\n    = Maximum total monthly debt\n    - every other committed payment\n      (car, cards, personal loans, school fees where counted)\n\nFrom a payment back to a loan\n  Loan = Payment x (1 - (1 + r)^-n) / r\n  where r is the monthly rate and n the number of months\n\nYour capacity is the LOWER of the two, always.\n\nThe stress test underneath both\n  Banks qualify the payment at a rate above the one they quote.\n  Re-run the payment two percentage points higher and confirm\n  it still clears, because that is the test being applied.",
  "failureModes": [
  ...
  "whenToUse": "Before viewing anything, so the search is over properties you can actually finance rather than a shortlist that collapses at the application. Then again on any change to income or committed debt, because both tests move.",
  "sources": [
    {
    ...
    {
      "name": "UAE mortgage rules and LTV limits 2026",
      "url": "https://www.grovy.ae/uae-mortgage-rules-ltv-limits-2026/"
    },
    {
      "name": "UAE mortgage regulations",
      "url": "https://bank-uae.com/uae-mortgage-regulations/"
    }
  ]
}
---
...
**And it is the one that gets understated**, because the deposit is not the only cash required. The transfer fee, agency commission, trustee fee and mortgage registration are paid on top and **cannot be borrowed**. A buyer who has saved exactly the headline deposit is short by the whole of [transaction cost drag](/playbooks/transaction-cost-drag/), which in Dubai runs to several percent of the price before anyone has moved in.
...
It is the same discipline the [cash on cash return](/playbooks/cash-on-cash/) framework applies from the other direction: a deal that only works at today's rate is a deal with no room in it.
